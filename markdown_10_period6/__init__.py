...
class C(BaseConstants):
    NAME_IN_URL = 'markdown_10_period6'
    PLAYERS_PER_GROUP = None
    NUM_ROUNDS = 4
    MAX = 20 # Maximum bid.
    NAMES = {
        1: ['L', 'Q'],  # First (highest) asset in market 1, second asset in market 2.
        2: ['K', 'B'],
        3: ['F', 'C'],
        4: ['C', 'O']
    }
...
class Intro(Page):
    def vars_for_template(player: Player):
        if (player.subsession.session.config['first_session'] == 1) and (player.round_number == 1):  # This app is the first
            player.participant.vars['round'] = 0                                                     # Initialize these variables.
...
class Bid1(Page):
    ...
    def vars_for_template(player: Player):
       ...
       return dict(
          image_path1= 'markdown_10_period6/markdown_{}_period6.jpg'.format(player.round_number),
          image_path2='markdown_10_period6/markdown_{}_graph.jpg'.format(player.round_number),
...
class CombinedResults(Page):
    ...
    def vars_for_template(player: Player):
        ...
        return dict(
            image_path= 'markdown_10_period6/markdown_{}_final.jpg'.format(player.round_number)
        )
